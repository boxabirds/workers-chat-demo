...
export class ChatRoom {
  ...
  async webSocketMessage(webSocket, msg) {
    try {
      ...
      // Check message against content policy using Cloudflare Workers AI
      const policyCheckResponse = await this.env.AI.run('@cf/meta/llama-3-8b-instruct', {
        messages: [
          {
            role: "user",
            content: `Check that the message is compliant with the content policy. If compliant, return OK, if not, return a suggested rephrasing of the message instead
<message>${data.message}</message>
<policy>Message must be respectful, polite, and not contain any racism or profanity.</policy>`
          }
        ]
      });
      ...
      let policyCheckResult = await policyCheckResponse.json();
      console.log("Policy check result:", JSON.stringify(policyCheckResult, null, 2));
  
      if (!policyCheckResult.choices || !policyCheckResult.choices[0] || !policyCheckResult.choices[0].text) {
        console.error("Unexpected response format from Cloudflare Workers AI API:", JSON.stringify(policyCheckResult, null, 2));
        webSocket.send(JSON.stringify({error: "An error occurred while checking the message against the content policy. Unexpected response format."}));
        return;
      }

      const checkedMessage = policyCheckResponse.choices[0].message.content.trim();

      if (checkedMessage !== "OK") {
        webSocket.send(JSON.stringify({error: "Message violates content policy. Suggested rephrasing: " + checkedMessage}));
        return;
      }
